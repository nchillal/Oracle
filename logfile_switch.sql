...
begin 
	SELECT (sysdate-COMPLETION_TIME)*24*60 INTO v_arch_log_idle_mins FROM v$archived_log WHERE sequence#=(SELECT max(sequence#) FROM v$log_history) AND dest_id=1;
